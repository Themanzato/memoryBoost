...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($alert) && !empty($dataToUpdate)) {
            $updateQuery = "UPDATE Control SET " . implode(", ", array_map(function ($key, $value) {
                return "$key = '$value'";
            }, array_keys($dataToUpdate), $dataToUpdate)) . " WHERE ID_Paciente = $id_paciente";

            $alert = mysqli_query($conn, $updateQuery) ? 'Datos actualizados correctamente' : 'Error al actualizar los datos';
        }
    }

    $query_consulta = mysqli_query($conn, "SELECT c.Diagnostico, p.Nombre AS NombrePaciente
    FROM Control c
    JOIN paciente p ON p.ID_paciente =  $id_paciente");

    if (mysqli_num_rows($query_consulta) > 0) {
        $data_consulta = mysqli_fetch_assoc($query_consulta);
        $nombre_paciente = $data_consulta['NombrePaciente'] ?? 'Nombre no disponible';
    } else {
...
<head>
    ...
    <title>Modificacion de Control: <?php echo $nombre_paciente; ?></title>
        ...
        <form method="post" id="updateForm">
            <table>
                <?php foreach ($data_consulta as $key => $value) : ?>
                    <?php if (in_array($key, ['Diagnostico'])) : ?>
                        <tr>
